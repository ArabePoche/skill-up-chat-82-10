 ...
 *
 * NB: on n'utilise PAS le composant Dialog de shadcn car son portail asynchrone
 * empêche canvasRef.current d'être disponible lors de l'init Fabric.js.
 * On utilise un overlay fixe directement dans le DOM.
 */
...
const StickerEditorModal: React.FC<Props> = ({ file, onConfirm, onCancel, removeBg }) => {
  ...
  const PencilBrushRef    = useRef<any>(null);
  ...
  /* Signale que le composant est monté (et donc le canvas est dans le DOM) */
  useEffect(() => {
    setMounted(true);
  ...
  /* ── Charge un dataUrl dans le canvas ── */
  const placeImage = useCallback(async (dataUrl: string, FabricImage: any, canvas: any) => {
    const fabImg: any = await FabricImage.fromURL(dataUrl, { crossOrigin: 'anonymous' });
  ...
  /* ── Init Fabric.js — attend que mounted === true (canvas dans le DOM) ── */
  useEffect(() => {
    if (!mounted || !file || !canvasRef.current) return;
    ...
    (async () => {
      try {
        ...
        console.error('StickerEditorModal: init canvas échoué', err);
  ...
  useEffect(() => {
    ...
    if (tool === 'draw' || tool === 'eraser') {
      ...
      if (PencilBrushRef.current) {
        ...
        brush.color = tool === 'draw' ? color : 'rgba(0,0,0,1)';
  ...
  return (
    /* overlay fixe — pas de portail asynchrone, le canvas est immédiatement dans le DOM */
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div
        ref={containerRef}
        className="bg-white rounded-2xl shadow-2xl w-full max-w-[560px] flex flex-col overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        {/* ── en-tête ── */}
        <div className="flex items-center gap-2 px-5 py-3.5 border-b bg-slate-50">
        ...
        {/* ── barre d'outils ── */}
        <div className="flex items-center gap-1.5 px-4 py-2 bg-white border-b flex-wrap">
          <TooltipProvider delayDuration={300}>
            ...
            {COLORS.map((c) => (
              <button
                ...
                onClick={() => setColor(c)}
                title={c}
                className={`w-5 h-5 rounded-full border-2 transition-transform hover:scale-110 ${color === c ? 'border-violet-500 scale-110' : 'border-slate-200'}`}
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <label className="w-5 h-5 rounded-full border-2 border-dashed border-slate-300 cursor-pointer flex items-center justify-center hover:border-violet-400 overflow-hidden relative">
            ...
            <div className="flex items-center gap-2 w-20">
              <span className="text-xs text-slate-500 shrink-0">px</span>
        ...
        <div
          ...
          style={{ height: MAX_H + 24 }}
        >
          <canvas ref={canvasRef} style={{ display: 'block', touchAction: 'none' }} />

          {/* overlay suppression fond */}
          {removingBg && (
        ...
        <div className="flex items-center justify-between gap-3 px-4 py-3 bg-slate-50 border-t">
          <button
            ...
            disabled={removingBg}
            className={`flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-medium border transition-colors disabled:opacity-50 ${
              bgRemoved
          ...
          <div className="flex gap-2">
            <Button variant="outline" size="sm" className="rounded-xl" onClick={onCancel} disabled={isExporting}>
              <X className="h-4 w-4 mr-1" /> Passer
            </Button>
            <Button
              size="sm"
