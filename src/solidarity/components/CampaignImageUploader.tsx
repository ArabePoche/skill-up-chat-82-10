/**
 * Composant d'upload d'image pour les cagnottes solidaires
...
const CampaignImageUploader: React.FC<CampaignImageUploaderProps> = ({ imageUrl, onImageChange, campaignTitle }) => {
  const { user } = useAuth();
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      console.error('Erreur upload image cagnotte:', err);
    ...
    <div className="space-y-2">
      ...
      {imageUrl ? (
        <div className="relative w-full h-40 rounded-lg overflow-hidden border border-border">
          <img
            src={imageUrl}
            alt={campaignTitle ? `Image de ${campaignTitle}` : 'Aperçu de la cagnotte'}
