...
export const useAgentAuth = (shopId?: string, options?: UseAgentAuthOptions) => {
    ...
    // Entree dans la zone securisee (gestion boutique): demander le PIN.
    useEffect(() => {
    ...
    // Verrouille la session quand l'app passe en arrière-plan, uniquement en zone securisee.
    useEffect(() => {
        if (!lockScopeActive) {
            return;
        }
        ...
        const handleVisibilityChange = () => {
            if (document.visibilityState === 'hidden' && activeAgent?.isUnlocked) {
                lock();
            }
        };
