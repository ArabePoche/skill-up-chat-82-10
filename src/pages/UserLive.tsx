...
import { LiveTeachingStudioRunner } from '@/live/components/studio-live';
import LiveScreenManager, { buildScreenFromStudio } from '@/live/components/LiveScreenManager';
import FollowButtonInline from '@/live/components/FollowButtonInline';
import LiveCommentsOverlay from '@/live/components/LiveCommentsOverlay';
import LiveControlsBar from '@/live/components/LiveControlsBar';
import LiveCreatorReportDialog from '@/live/components/LiveCreatorReportDialog';
...
import {
  ...
  syncPresenceEntries,
  WhiteboardHistoryMap,
} from '@/live/lib/userLiveShared';
...
const UserLive: React.FC = () => {
  ...
  const handleSatisfactionSubmit = async (isSatisfied: boolean, refundRequested: boolean) => {
    ...
    try {
      ...
      toast.error('Impossible denvoyer votre retour. Veuillez réessayer.');
    } finally {
  ...
  return (
    <div className={`relative flex h-[100dvh] min-h-screen w-full bg-black text-white overflow-hidden ${isStudioMode ? 'flex-col md:flex-row' : 'flex-col'}`}>
          ...
          <div className="flex-[1] md:flex-[1.5] relative bg-black border-l border-white/5 h-[40vh] md:h-full">
            ...
            <div className="absolute bottom-16 left-2 right-2 z-20 pointer-events-auto">
              <div className="max-h-[140px] overflow-y-auto space-y-1.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden overscroll-contain touch-pan-y">
                {messages.map((msg) => <LiveMessageItem key={msg.id} message={msg} hostId={stream.host_id} />)}
              </div>
            </div>
          </div>
        </>
      ...
      <div className="absolute inset-0 z-10 pointer-events-none flex flex-col justify-end">
        ...
        {publicLiveScreen?.type !== 'teaching_studio' && (
          <LiveCommentsOverlay
            ...
            isCollapsed={areCommentsCollapsed}
            onExpand={() => setAreCommentsCollapsed(false)}
            onTouchStart={handleCommentsTouchStart}
            onTouchEnd={handleCommentsTouchEnd}
            commentsScrollRef={commentsScrollRef}
            hasParticipantPanel={acceptedParticipants.length > 0 || isAcceptedParticipant}
          />
        )}
      </div>
      ...
      {isHost && stream && (
        <LiveRegistrantsDialog
          open={showRegistrantsPanel}
          onOpenChange={setShowRegistrantsPanel}
          stream={stream}
      ...
      {isHost && stream && (
        <LiveCreatorReportDialog
          open={showCreatorReport}
          stream={stream}
          reportEndedAt={reportEndedAt}
          reportViewerCount={reportViewerCount}
          reportPaidEntryCount={reportPaidEntryCount}
          liveGiftTotals={liveGiftTotals}
          onCloseToProfile={() => {
            setShowCreatorReport(false);
            navigate('/profil');
          }}
        />
      )}
