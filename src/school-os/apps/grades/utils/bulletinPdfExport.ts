...
// Helper function to reverse text for RTL display (basic approach for jsPDF)
const processArabicText = (text: string): string => {
  if (!containsArabic(text)) return text;
  // Reverse for basic RTL support in jsPDF
...
export const exportBulletinsToPdf = async ({
  ...
  const showPhoto = template?.show_photo ?? true;
  ...
  bulletins.forEach((student, index) => {
    ...
    if (hasArabicSubjects) {
      // RTL-friendly layout: Appreciation on left, Subject name on right
    ...
    student.grades.forEach((grade) => {
      ...
      const scoreText = grade.isAbsent ? 'ABS' : (grade.score !== null ? grade.score.toString() : '-');
      
      doc.setFontSize(9);
      doc.setTextColor(0, 0, 0);

      if (hasArabicSubjects && containsArabic(grade.subjectName)) {
        // For Arabic subjects - draw text from right
        doc.text(subjectAppreciation, 17, y);
        doc.text('20', 57, y);
        doc.text(grade.coefficient.toString(), 73, y);
        doc.text(scoreText, 92, y);
        // Draw Arabic subject name (reversed for basic RTL)
        doc.text(processArabicText(grade.subjectName.substring(0, 30)), 175, y, { align: 'right' });
      } else {
        doc.text(grade.subjectName.substring(0, 28), 17, y);
        doc.text(scoreText, 88, y);
