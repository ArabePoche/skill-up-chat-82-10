/**
 * Modal affichant l'historique des paiements d'une famille avec actions
 */
...
import { useFamilyPaymentHistory } from '../hooks/useFamilyPaymentHistory';
import { Edit2, Trash2, Calendar, DollarSign, FileText, User } from 'lucide-react';
...
interface FamilyPaymentHistoryModalProps {
  open: boolean;
...
export const FamilyPaymentHistoryModal: React.FC<FamilyPaymentHistoryModalProps> = ({
  open,
  onOpenChange,
  familyId,
  familyName,
}) => {
  const { data: payments = [], isLoading } = useFamilyPaymentHistory(familyId);
    ...
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-3xl max-h-[90vh]">
          <DialogHeader>
            <DialogTitle>Historique des paiements familiaux</DialogTitle>
            <DialogDescription>
              Tous les paiements de la famille {familyName}
            </DialogDescription>
          </DialogHeader>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-8 text-muted-foreground">
              Aucun paiement enregistré pour cette famille.
            </div>
          ) : (
            <>
              {/* Résumé */}
