...
-- Corriger le trigger d'initialisation des leçons avec une meilleure gestion d'erreurs
...
-- Recréer la fonction avec une gestion d'erreurs améliorée
...
BEGIN
    ...
    IF NEW.status = 'approved' AND OLD.status != 'approved' THEN
        ...
        IF first_lesson_id IS NOT NULL THEN
            -- Insérer le progrès de la leçon avec le bon statut ENUM
            INSERT INTO user_lesson_progress (user_id, lesson_id, status, exercise_completed)
            VALUES (NEW.user_id, first_lesson_id, 'not_started'::lesson_status, FALSE)
            ON CONFLICT (user_id, lesson_id) DO UPDATE SET
                status = 'not_started'::lesson_status,
                exercise_completed = FALSE;
            
            RAISE NOTICE 'Initialized lesson progress % for user %', first_lesson_id, NEW.user_id;
            ...
            END IF;
        ELSE
            RAISE NOTICE 'No lessons found for formation %', NEW.formation_id;
        END IF;
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING 'Error in handle_enrollment_approval: %', SQLERRM;
