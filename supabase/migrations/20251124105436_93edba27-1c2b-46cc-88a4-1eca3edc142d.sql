-- Corriger la fonction approve_school_join_request pour mapper les valeurs teacher_type
...
BEGIN
  ...
  v_request_data := v_request.request_data;
  ...
  v_teacher_type := CASE 
    WHEN v_request_data->>'teacherType' = 'generalist' THEN 'generaliste'
    WHEN v_request_data->>'teacherType' = 'specialist' THEN 'specialiste'
    ELSE v_request_data->>'teacherType'
  END;

  -- Extraire specialty et preferredGrade
  IF v_request_data ? 'specialty' THEN
    v_specialties := ARRAY[v_request_data->>'specialty'];
  END IF;

  v_preferred_grade := v_request_data->>'preferredGrade';
